...
test("FreeHire discovery uses the fixed HTTPS endpoint and refuses redirects", async () => {
  ...
  try {
    ...
    expect(requested).toEqual([{
      url: "https://freehire.dev/api/v1/jobs/search?q=data+center+technician&limit=5&offset=0&semantic_ratio=0&countries=DE&cities=Frankfurt&cities=Eschborn",
      redirect: "error",
    }]);
  } finally {
    globalThis.fetch = originalFetch;
...
test("FreeHire discovery caps fixture imports at 50 jobs", async () => {
  ...
  try {
    ...
    expect(results.some((result) => result.sourceId === "freehire:cap-50")).toBe(false);
  } finally {
    globalThis.fetch = originalFetch;
    db.close();
  }
